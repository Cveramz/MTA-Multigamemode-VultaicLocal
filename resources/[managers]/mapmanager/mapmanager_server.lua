...
function loadMapData(arenaElement, resourceName)
	...
	if mapCache[resourceName] then
		...
		-- Self-hosted servers may not have the external HTTP cache used by the
		-- original Vultaic deployment. Keep the parsed elements in memory so they
		-- can be sent through MTA's event transport instead.
		mapCache[resourceName].elements = elements
	...
	if type(players) == "table" then
		if #players > 0 then
			...
			for i, player in pairs(players) do
				triggerClientEvent(player, "downloadMap", resourceRoot, resourceName, mapCache[resourceName].elementsURL, mapCache[resourceName].storageURL, mapCache[resourceName].key, mapCache[resourceName].scripts, mapCache[resourceName].files, mapCache[resourceName].info, mapCache[resourceName].settings, mapCache[resourceName].elements)
			end
			return true
		else
			return
