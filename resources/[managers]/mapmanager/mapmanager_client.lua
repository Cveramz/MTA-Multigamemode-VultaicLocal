...
function downloadMap(resourceName, elementsURL, storageURL, key, scripts, files, info, settings, directElements)
	unloadMap()
	...
	if type(resourceName) == "string" then
		...
		if type(directElements) == "table" then
			outputDebugString("Loading elements received directly for "..resourceName, 0, 255, 255, 255)
			loadMap(directElements)
			mapWasReadFromCache = true
		end
		if not mapWasReadFromCache and type(key) == "string" then
