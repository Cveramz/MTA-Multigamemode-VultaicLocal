...
function()
	...
	triggerServerEvent("login:onClientReady", localPlayer)
...
-- The server sends this once the client has finished downloading this resource.
-- This avoids losing the login initialization on newer MTA clients while many
-- resources are being downloaded at the same time.
addEvent("login:forceShow", true)
addEventHandler("login:forceShow", resourceRoot, login.check)
...
function loadDetails()
	...
	if type(details) == "table" then
		local username, password = details.username, decodeString("base64", details.password)
...
function saveDetails(username, password)
	if username and password then
		...
		if file then
			fileWrite(file, toJSON({username = username, password = encodeString("base64", password)}))
			fileClose(file)
		end
	end
end
